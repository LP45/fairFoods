...
// const DB_URL = 'mongodb+srv://amber:moondance@cluster0-luzot.mongodb.net/<dbname>?retryWrites=true&w=majority';
...
app.post('/order', (req, res) => {
    ...
    db_handler.collection(ORDER_COLLECTION).insertOne(my_object, (err, result) => {
        if (err) {
            ...
            confirmation_message = `Order entered by ${clientName} for ${bags} bags.`;
            // or you can do the below option
            // confirmation_message = "Order entered by " + clientName + " for " + bags + " bags.";
            res.redirect('/');
        }
    });
    // res.redirect('/');is the incorrect spot for this
});
